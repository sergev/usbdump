...
void usb_sniff(int bus, int address)
{
    ...
    kbuf_len = ioctl(fd, MON_IOCQ_RING_SIZE);
    if (kbuf_len <= 0) {
        printf("failed to determine kernel USB buffer size: %s\n", strerror(errno));
        return;
    }

    mbuf = mmap(NULL, kbuf_len, PROT_READ, MAP_SHARED, fd, 0);
    if (mbuf == MAP_FAILED) {
...
int check_device(struct dirent *de, char *vidpid, int *bus, int *address)
{
    ...
    if (fd < 0)
        return 0;
    buf[4] = '\0';
    if (read(fd, buf, 4) != 4 || strncmp(vid, buf, 4) != 0) {
        close(fd);
        return 0;
    }
    close(fd);
    ...
    if (fd < 0)
        return 0;
    buf[4] = '\0';
    if (read(fd, buf, 4) != 4 || strncmp(vidpid+5, buf, 4) != 0) {
        close(fd);
        return 0;
    }
    close(fd);
